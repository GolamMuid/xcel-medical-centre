...
function About() {
	...
	const {
		...
		queryFn: async () => {
			const res = await axios.get(
				`${process.env.REACT_APP_API_URL}/api/get-all-abouts`
				// `/api/set-service`
			);
	...
	const data = useMemo(() => aboutData, [aboutData]);
	...
	return (
		<>
			{isLoading ? (
				...
				<div>
					<div className="search-export-container flex flex-col gap-4 pb-4 md:flex-row items-center px-2">
