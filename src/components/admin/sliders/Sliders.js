...
import SlidersAdd from "./SlidersAdd";
import SlidersEdit from "./SlidersEdit";
		...
		},
		{
			...
			Cell: (row) => (
				<div className="flex justify-center ">
					<img className="h-12" src={row.row.original.sliderImage} alt="" />
				</div>
			),
			Filter: false,
			disableSortBy: true,
	...
	const {
		...
		queryFn: async () => {
			const res = await axios.get(
				`${process.env.REACT_APP_API_URL}/api/get-all-slider`
				// `/api/set-service`
			);
		...
		<>
			{isLoading ? (
				...
				<div>
					...
					<SlidersAdd
						sliderAdd={sliderAdd}
						setSliderAdd={setSliderAdd}
						refetch={refetch}
					/>
					<SlidersEdit
						sliderEdit={sliderEdit}
						setSliderEdit={setSliderEdit}
