...
function PhotosEdit({
	...
	useEffect(() => {
		...
		setImage(selectedRowData?.photoGallery);
	...
	const onSubmit = async (data) => {
		...
		try {
			const response = await axios.post(
				`${process.env.REACT_APP_API_URL}/api/set-photo-gallery/${selectedRowData.id}`,
				formData
			);
	...
	return (
		<div>
			<input
				...
				checked={photosEdit}
				onChange={(e) =>
					e.target.checked ? setPhotosEdit(false) : setPhotosEdit(true)
				}
			/>
