...
function Photos() {
	const [photosAdd, setPhotosAdd] = useState(false);
	const [photosEdit, setPhotosEdit] = useState(false);
		...
		},
		{
			...
			Cell: (row) => (
				<div className="flex justify-center ">
					<img className="h-12" src={row.row.original.photoGallery} alt="" />
				</div>
			),
			Filter: false,
			disableSortBy: true,
	...
	const {
		...
		queryFn: async () => {
			const res = await axios.get(
				`${process.env.REACT_APP_API_URL}/api/get-all-gallery-photos`
			);
	...
	const data = useMemo(() => photosData, [photosData]);
	...
	return (
        ...
        >
          + Add
					...
					<PhotosAdd
						photosAdd={photosAdd}
						setPhotosAdd={setPhotosAdd}
						refetch={refetch}
					/>

					<PhotosEdit
						photosEdit={photosEdit}
						setPhotosEdit={setPhotosEdit}
